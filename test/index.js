...
for (var i = 0; i < blobsPOC2.length; i++) {
  var blob = blobsPOC2[i]
  var hasher = swarmhash({ mode: 'poc2' })
  tape('POC2: hashing ' + blob[0] + ' zeroes', function (t) {
    t.plan(1)
    t.equal(hasher(Buffer.alloc(blob[0])).toString('hex'), blob[1])
  })
}

for (var i = 0; i < blobsPOC3.length; i++) {
  var blob = blobsPOC3[i]
  var hasher = swarmhash({ mode: 'poc3' })
  tape('POC3: hashing ' + blob[0] + ' zeroes', function (t) {
    t.plan(1)
    t.equal(hasher(Buffer.alloc(blob[0])).toString('hex'), blob[1])
  })
}
